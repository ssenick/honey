...
export function headerScroll() {
  ...
  const cmBtm = document.querySelector('.content-menu__button');
  ...
  document.addEventListener('windowScroll', function (e) {
    ...
    if (contentMenu) {
      const btnPosition = cmBtm.getBoundingClientRect().top;
      if (btnPosition >= headerHeight + 20) {
        if (triger) {
          triger = false;
          contentMenu.classList.contains('_hide') ? contentMenu.classList.remove('_hide') : null;
        }
      } else {
        if (!triger) {
          triger = true;
          contentMenu.classList.add('_hide');
        }
      }
    }
